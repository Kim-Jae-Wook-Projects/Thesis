...
class RouterConfig:
    ...
    trigger_window_sec: float = 60.0
    track3_cooldown_sec: float = 120.0
...
class TrackRouter:
    ...
    def _prune(self, now_ts: float) -> None:
        w = float(self.cfg.trigger_window_sec)
        while self._trig_hits and (float(now_ts) - float(self._trig_hits[0])) > w:
            self._trig_hits.popleft()
